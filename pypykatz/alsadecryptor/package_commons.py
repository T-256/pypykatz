...
class PackageDecryptor:
	...
	async def walk_list(self, entry_ptr, callback, max_walk = 255, override_ptr = None):
		...
		max_walk = max_walk
		await self.log_ptr(entry_ptr.value, 'List entry -%s-' % entry_ptr.finaltype.__name__ if not override_ptr else override_ptr.__name__)
		while True:
			if override_ptr:
			...
			if entry.Flink.value != 0 and entry.Flink.value not in entries_seen and max_walk != 0:
				...
				await self.log_ptr(entry.Flink.value, 'Next list entry -%s-' % entry.Flink.finaltype.__name__ if not override_ptr else override_ptr.__name__)
